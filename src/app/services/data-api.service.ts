...
export class DataApiService {
  ...
  getAllBooksOffers() {
    this.booksCollection = this.afs.collection('books', ref => ref.where('oferta', '==', 'SI'));
    return this.books = this.booksCollection.snapshotChanges()
    .pipe(map(changes => {
      return changes.map(action => {
        const data = action.payload.doc.data() as BookInterface;
  ...
  getAllOwnedBooks(uid: string) {
    console.log('in getAllOwnedBooks');
    console.log('user id', uid);
    this.booksCollection = this.afs.collection('books', ref => ref.where('userUid', '==', uid));
    return this.books = this.booksCollection.snapshotChanges()
    .pipe(map(changes => {
      return changes.map(action => {
        const data = action.payload.doc.data() as BookInterface;
